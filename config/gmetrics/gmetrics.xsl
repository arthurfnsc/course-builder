...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:key name="packageSummary" match="PackageSummary/MetricResult"
		use="@name" />
	...
	<xsl:template match="GMetrics">
		<html>
			...
			<body>
				...
				<p>
					XSLT created by
					<a href="http://www.mrhaki.com">Hubert A. Klein Ikkink (mrhaki)</a>
				</p>
		...
		<ul id="summary_stats">
				...
				<strong>
					<xsl:call-template name="display_value">
						<xsl:with-param name="value"
							select="MetricResult[@name = 'ABC']/@average" />
					</xsl:call-template>
				</strong>
				<span>ABC</span>
			</li>
			<li>
				<strong>
					<xsl:call-template name="display_value">
						<xsl:with-param name="value"
							select="MetricResult[@name = 'CyclomaticComplexity']/@average" />
					</xsl:call-template>
				</strong>
			...
			<li>
				<strong>
					<xsl:call-template name="display_value">
						<xsl:with-param name="value"
							select="MetricResult[@name = 'MethodLineCount']/@average" />
					</xsl:call-template>
				</strong>
			...
			<li>
				<strong>
					<xsl:call-template name="display_value">
						<xsl:with-param name="value"
							select="MetricResult[@name = 'ClassLineCount']/@average" />
					</xsl:call-template>
				</strong>
	...
	<xsl:template match="Class" mode="full">
		<xsl:variable name="classLineCount"
			select="MetricResult[@name = 'ClassLineCount']" />
		<xsl:variable name="methodLineCount"
			select="MetricResult[@name = 'MethodLineCount']" />
		<xsl:variable name="abc" select="MetricResult[@name = 'ABC']" />
		<xsl:variable name="cyclomaticComplexity"
			select="MetricResult[@name = 'CyclomaticComplexity']" />
		<a name="c-{@name}"></a>
		<h2>
			Class
			<xsl:value-of select="@name" />
						...
						<td>
							...
							<dl>
								...
								<dd>
									<xsl:call-template name="display_value">
										<xsl:with-param name="value"
											select="$methodLineCount/@average" />
									</xsl:call-template>
								</dd>
								<dt>Maximum LC:</dt>
								<dd>
						...
						<td>
							...
							<p>
								<strong>
									<xsl:call-template name="display_value">
										<xsl:with-param name="value"
											select="$cyclomaticComplexity/@average" />
									</xsl:call-template>
								</strong>
							</p>
							<dl>
								...
								<dd>
									<xsl:call-template name="display_value">
										<xsl:with-param name="value"
											select="$cyclomaticComplexity/@maximum" />
									</xsl:call-template>
								</dd>
							</dl>
						</td>
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				...
				<xsl:for-each select="Method">
					...
					<tr>
						...
						<td>
							<xsl:value-of
								select="MetricResult[@name = 'CyclomaticComplexity']/@total" />
						</td>
						<td>
